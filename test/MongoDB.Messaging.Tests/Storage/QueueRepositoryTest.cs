...
namespace MongoDB.Messaging.Tests.Storage
{
    ...
    public class QueueRepositoryTest
    {
        ...
        public async void SaveFind()
        {
            ...
            var m = await repo.Save(message).ConfigureAwait(false);
            m.Should().Be(message);
        ...
        public async void FindOne()
        {
            ...
            await repo.Save(message).ConfigureAwait(false);

            Expression<Func<Message, bool>> criteria = (m) => m.State == MessageState.None && m.Result == MessageResult.None;

            var one = await repo.FindOne(criteria).ConfigureAwait(false);
        ...
        public async void FindAll()
        {
            ...
            var list = await repo.FindAll(criteria).ConfigureAwait(false);
        ...
        public async void SaveDelete()
        {
            ...
            long count = await repo.Delete(key).ConfigureAwait(false);
        ...
        public async void DeleteOne()
        {
            ...
            long count = await repo.DeleteOne(criteria).ConfigureAwait(false);
            count.Should().BeGreaterThan(0);
        }
        ...
        public async void Count()
        {
            ...
            long count = await repo.Count(criteria).ConfigureAwait(false);
            count.Should().BeGreaterThan(0);
        }
        ...
        public async void EnqueueDequeue()
        {
            ...
            var m = await repo.Enqueue(message).ConfigureAwait(false);
            m.Should().Be(message);
            ...
            var dequeued = await repo.Dequeue().ConfigureAwait(false);
            ...
            await repo.UpdateStatus(key, "Loading Test Data ...").ConfigureAwait(false);

            await repo.MarkComplete(key, MessageResult.Successful, "Test Complete").ConfigureAwait(false);

            var completed = await repo.Find(key).ConfigureAwait(false);
